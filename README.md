...
|---|---|---|---|
| `adsb/aircraft/{hex}` | full JSON state | yes | any field change |
| `adsb/aircraft/{hex}` | empty | yes | aircraft not seen for `STALE_AFTER_SEC` |
| `adsb/flight/{flight}/{hex}` | full JSON state | no | flight callsign known + state changed |
| `adsb/type/{type}/{hex}` | full JSON state | no | on every change (`adsb_icao`, `mlat`, `mode_s`, ...) |
| `adsb/events/emergency/{hex}` | full JSON state | no | squawk in `7500`/`7600`/`7700` |
| `adsb/status` | `online` / `offline` | yes | LWT + connect |

The prefix (`adsb` above) is configurable via `MQTT_TOPIC_PREFIX`. Callsigns
are uppercased and any character outside `[A-Z0-9_-]` is replaced with `_`
in topic segments.
...
```sh
mosquitto_sub -h localhost -t 'adsb/status'
mosquitto_sub -h localhost -t 'adsb/aircraft/+' -v
mosquitto_sub -h localhost -t 'adsb/events/emergency/+' -v
mosquitto_sub -h localhost -t 'adsb/flight/BAW+/+' -v
mosquitto_sub -h localhost -t 'adsb/aircraft/+' -C 1 | jq .
```
